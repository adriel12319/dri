...
class TelegramBot:
    ...
    def start(self):
        ...
        @TelegramBotErrorHandler.command_handler(self.__logger, function_usage="/formato")
        def formats(update, context):
            ans = f"Os Formatos disponiveis são: {Formats.to_string()}\nEsses São os tamanhos preferenciais porém podem não estar disponível para seu video."
            update.message.reply_text(ans)
        ...
        @TelegramBotErrorHandler.command_handler(self.__logger, function_usage="/video <youtube link> ou /video 480p <youtube link>")
        def video(update, context):
            ...
            if(len(context.args) == 2):
                ...
                if(quality not in Formats.video_formats):
                    self.__logger.warning(f"Formato do video {quality} não é suportado")
                    raise ValueError()
            ...
            update.message.reply_text("⬇️📽️ Começando Download...")
        ...
        dp.add_handler(CommandHandler("formato", formats))
