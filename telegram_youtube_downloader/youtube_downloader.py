...
class YoutubeDownloader:
    ...
    def __download(self, url, options):
        ...
        try:
            with yt.YoutubeDL(options) as ydl:
                ...
                max_duration = options["max_duration_segundos"]
                dl_type = options["dl_type"]
                if(meta["duration"] > max_duration):
                    raise DownloadError(f"Maximum allowed video duration for '{dl_type}' download is {str(datetime.timedelta(segundos=max_duration))}")
    ...
    def get_max_video_duration(self):
        return str(datetime.timedelta(segundos=self.__download_options["max_video_duration_segundos"]))

    def get_max_audio_duration(self):
        return str(datetime.timedelta(segundos=self.__download_options["max_audio_duration_segundos"]))
